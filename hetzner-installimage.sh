...
if [[ -n "$VALIDHOSTNAME" ]]; then
  ...
  /root/.oldroot/nfs/install/installimage -a -p /boot:ext3:1G,/:ext4:999G -r yes -l 1 -b grub -d sda,sdb -i /root/.oldroot/nfs/install/../images/Ubuntu-1404-trusty-64-minimal.tar.gz -x hetzner-postinstall.sh -n ${SERVERNAME}
fi
